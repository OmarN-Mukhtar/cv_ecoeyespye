...
def setup_model(num_classes, atrous_rates=None):
...
def find_best_dilations(train_loader, val_loader, num_classes, logger):
    dilation_configs = [
        [3, 6, 12],
        [6, 12, 18],
        [2, 4, 8],
    ...
    for rates in dilation_configs:
        ...
        model = setup_model(num_classes, atrous_rates=rates)
        ...
        _, _, val_iou = validate_epoch(model, val_loader, criterion, 'cuda', num_classes, logger)
...
def main():
    ...
    best_dilations = find_best_dilations(train_loader_grid_search, val_loader, config['num_classes'], logger)
    ...
    model = setup_model(config['num_classes'], atrous_rates=best_dilations)
    criterion = nn.CrossEntropyLoss()
